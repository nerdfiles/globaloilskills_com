...
var xhr=jQuery.ajax({data:options.data,dataType:options.dataType||'json',type:options.type||'POST',url:options.url,dataFilter:function(data,type)
{if(type=='json')
...
{candidate=data.slice(firstQuote,lastQuote-firstQuote);return candidate;}
else if(firstCurly&&(firstCurly!=lastCurly)&&(!lastQuote||(lastCurly>lastQuote)))
{candidate=data.slice(firstCurly,lastCurly-firstCurly);return candidate;}}}
